...
export const CalendarView: React.FC<CalendarViewProps> = ({ logs }) => {
  ...
  const getLogsForDate = (date: Date) => {
    return logs.filter((log) => {
      ...
      return logDate.getDate() === date.getDate() && logDate.getMonth() === date.getMonth() && logDate.getFullYear() === date.getFullYear()
    })
  }
  ...
  const renderDayCell = (date: Date | null, index: number) => {
    ...
    return (
      ...
      >
        <div className={`text-right text-sm mb-1 ${isToday ? 'font-bold text-blue-600' : 'text-gray-700'}`}>{date.getDate()}</div>
        <div className="flex-grow flex flex-col gap-1 overflow-hidden">
          {inCount > 0 && <div className="text-xs bg-green-100 text-green-800 px-1 rounded truncate">入: {inCount}件</div>}
          {outCount > 0 && <div className="text-xs bg-orange-100 text-orange-800 px-1 rounded truncate">出: {outCount}件</div>}
        </div>
      </div>
    )
  }
    ...
    <div className="bg-white shadow rounded-lg p-4">
      ...
      {selectedDate && (
        <div className="mt-6 border-t pt-4">
          <h3 className="font-bold text-lg mb-2">{selectedDate.toLocaleDateString()} の入出庫履歴</h3>
              ...
              {selectedLogs.map((log) => (
                <div key={log.id} className="flex items-center justify-between p-3 bg-gray-50 rounded border">
                  ...
                  <div className="text-sm text-gray-600">
                    <span className="font-bold text-gray-900">{log.quantity}</span> 個<span className="ml-3 text-xs text-gray-400">{new Date(log.date).toLocaleTimeString()}</span>
                  </div>
                </div>
              ))}
            </div>
